...
int main() {
    ...
    // Variables for use in assembly block
    unsigned matrix_value, kernel_value;    
    unsigned sum;                           
    unsigned matrix_offset;                 
    unsigned tmp_si, tmp_di;                
    ...
    asm volatile(
        "xor %%esi, %%esi \n\t"        // Clear ESI
        "xor %%edi, %%edi \n\t"        // Clear EDI
        "mov %1, %%ebx \n\t"           // Move matrix pointer to EBX
        "mov 4(%1), %%eax \n\t"        // Move matrix[1] to EAX
        
        : "=r" (sum)                   // Output
        : "r" (matrix),                // Inputs
        "r" (kernel),
        "r" (matrix_offset)
        : "esi", "edi", "ebx", "eax"   // Clobber list
    );
